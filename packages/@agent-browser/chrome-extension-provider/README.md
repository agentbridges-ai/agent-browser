...
The status output includes the unpacked extension path, bridge protocol version, daemon port, connected profile ids, extension id/version, and the immutable extension build identity. It deliberately excludes tab URLs, page titles, session owners, and control phases. Extension and daemon package versions remain diagnostic, while a supervised Nexolyra daemon additionally requires the exact provider git tree identity embedded into the extension build. This prevents an older unpacked build with the same package version from satisfying readiness.
...
When Nexolyra owns the daemon lifecycle it starts the process with an internal supervision marker. `/health` exposes only bounded onboarding metadata so destructive development harnesses can refuse to restart an unmanaged daemon. Session inventory and control state are available only from the host-authenticated `/control/status` endpoint. The marker is operational metadata, not an authentication mechanism.

The daemon rejects `/bridge` upgrades unless the browser-supplied WebSocket Origin matches the pinned extension id, then checks the hello payload against the same id. Browser-origin requests are also rejected from host control routes. Every `/control/*` request requires a host secret delivered to the supervised daemon over an anonymous inherited file descriptor, never through the session environment, argv, logs, or page context. Agent-facing provider launches receive only a signed owner-and-route grant; the daemon exchanges it once for a short-lived nonce, inherits the owner's current fence, and gives every CDP session a fail-closed target scope. Session cleanup requires either host authentication or that exact bridge session's token. These checks prevent an ordinary web page, a different extension, or an uncredentialed local process from minting an ownerless Agent session or flipping a human takeover back to Agent. A custom development build can override the extension id explicitly. Source Mode still assumes a trusted single operating-system user; it is not a production sandbox against a process able to inspect or tamper with another same-user process.

For supervised launches, the same private descriptor also carries the expected public build identity. The extension derives this identity from the committed provider git tree at build time and includes it in the bridge hello. A missing or different identity is rejected before the profile becomes available; it is not a secret and does not weaken the separate host control credential.

The extension uses `chrome.debugger` as its session-scoped page-control transport and requests neither blanket `<all_urls>` access nor host permissions. The foreground onboarding page reaches the fixed loopback daemon only after Chrome's Local Network Access grant. Browser commands, screenshots, and event-driven Live frames continue through the debugger session attached to the task tab.
