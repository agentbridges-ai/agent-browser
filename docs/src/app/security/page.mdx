...
- **Chrome extension bridge permissions are powerful.** The optional Chrome extension bridge provider uses the extension `debugger` permission and a localhost daemon to control the current Chrome profile. Install it only in profiles where that level of local automation is acceptable.
...
The Chrome extension bridge provider is opt-in and only activates when selected with `--provider chrome-extension`. Its daemon listens on `127.0.0.1`, pins bridge WebSocket connections to the bundled extension identity, validates per-session CDP tokens, and treats `browser.close` as detach so it does not close the user's desktop Chrome.
