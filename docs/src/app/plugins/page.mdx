...
- Connect to a local bridge that returns a CDP WebSocket URL, such as the optional Chrome extension bridge provider
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>chrome-extension.manage</code></td><td><code>chrome-extension.status</code></td><td><code>plugin run chrome-extension chrome-extension.status</code></td><td><code>data</code></td></tr>
  </tbody>
</table>
...
## Chrome extension bridge provider

The optional `@agent-browser/chrome-extension-provider` package implements a local `browser.provider` plugin named `chrome-extension`. It starts or reuses a local bridge daemon, waits for the unpacked MV3 extension to connect, then returns a CDP WebSocket URL to agent-browser.
...
The bridge provider is opt-in. It does not change local Chrome launch, `--cdp`, `--auto-connect`, or `--profile` behavior when the provider is not selected.
